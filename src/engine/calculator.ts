...
// v1.2: Total price cap Rp 500.000.000 applied to NB, EB, TS, NH
// ─────────────────────────────────────────────
...
/**
 * Apply total price cap of Rp 500.000.000.
 * Returns { cappedTotal, capApplied, capNote }.
 */
function applyTotalCap(rawTotal: number): { cappedTotal: number; capApplied: boolean } {
  if (rawTotal > TOTAL_PRICE_CAP) {
    return { cappedTotal: TOTAL_PRICE_CAP, capApplied: true };
  }
  return { cappedTotal: rawTotal, capApplied: false };
}
...
export function calculateNBEB(input: NBEBInput): CalculationResult {
  ...
  if (bracket) {
    ...
    breakdown.push({
      label: `Biaya Sertifikasi (${area.toLocaleString('id-ID')} m²)`,
      amount: baseFee,
    });
  } else {
    // Area ≥ 150,000 m²
    ...
    const rawSurcharge = excessArea * NBEB_HIGH_AREA_RATE;
    surcharge = rawSurcharge; // will be capped at total level below

    breakdown.push({ label: `Biaya Dasar (≥ 150.000 m²)`, amount: baseFee });
    breakdown.push({
      label: `Biaya Tambahan (${excessArea.toLocaleString('id-ID')} m² × Rp 1.500)`,
...
export function calculateIS(input: ISInput): CalculationResult {
  ...
  if (area < IS_MIN_AREA) {
    throw new Error(`Luas area minimum untuk Interior Space adalah ${IS_MIN_AREA} m².`);
  }
...
export function calculateTS(input: TSInput): CalculationResult {
  ...
  if (area < TS_MIN_AREA) {
    throw new Error(`Luas area minimum untuk Transit Station adalah ${TS_MIN_AREA.toLocaleString('id-ID')} m².`);
  }
...
export function calculateHomesA(input: HomesAInput): CalculationResult {
  ...
  if (floorArea <= 100) {
    certFee = HOMES_A_SMALL_FEE;
    category = 'Kecil (≤ 100 m²)';
  } else if (floorArea <= 200) {
    certFee = HOMES_A_MEDIUM_FEE;
    category = 'Menengah (101–200 m²)';
  } else {
    certFee = HOMES_A_LARGE_FEE;
    category = 'Besar (> 200 m²)';
  }
  ...
  return {
    ...
    breakdown: [
      ...
      { label: 'Biaya Pendaftaran', amount: HOMES_REG },
      { label: 'Total', amount: total, isTotal: true },
    ],
    warnings: [],
    edgeCases: [],
    maxFeeForRing: RING_MAX['HOMES_A'],
  };
}
...
function getDiscountRate(activeRank: number): number {
  if (activeRank < HOMES_B_DISCOUNT.length) return HOMES_B_DISCOUNT[activeRank];
  return HOMES_B_DISCOUNT_REST;
}
...
export function calculateHomesB(input: HomesBInput): CalculationResult {
  ...
  if (capApplied) {
    breakdown.push({
      label: `Cap Diterapkan (maks. Rp 200.000.000)`,
      amount: certAfterCap,
      isCapped: true,
      isWarning: true,
      note: `Subtotal Rp ${subtotalCert.toLocaleString('id-ID')} dipangkas menjadi Rp 200.000.000`,
    });
    warnings.push(`Biaya sertifikasi dipangkas ke maksimum Rp 200.000.000 (subtotal asli: Rp ${subtotalCert.toLocaleString('id-ID')}).`);
  }

  breakdown.push({ label: 'Total Surcharge Audit', amount: totalSurcharge });
  breakdown.push({ label: 'Biaya Pendaftaran', amount: HOMES_REG });

  const total = certAfterCap + totalSurcharge + HOMES_REG;
...
export function calculateNH(input: NHInput): CalculationResult {
  ...
  if (area > NH_HIGH_AREA_THRESHOLD) {
    ...
    breakdown.push({
      label: `Biaya Tambahan (${excessHa.toFixed(3)} ha × Rp ${highAreaRate.toLocaleString('id-ID')}/ha)`,
      amount: Math.round(surcharge),
      isWarning: true,
    });
  ...
  return {
    scheme: schemeKey as 'NH_PLAN' | 'NH_BUILT',
    schemeName,
    total: cappedTotal,
    breakdown,
    warnings,
    edgeCases: [],
    maxFeeForRing: RING_MAX[schemeKey],
  };
}
